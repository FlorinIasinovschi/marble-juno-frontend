...
export const MyCollectedNFTs = ({ id }) => {
  const { client } = useSdk();
  const { getOwnedNfts, getOwnedNftCounts } = useSubquery();
  ...
  const getNfts = async (limit = 12) => {
    ...
    const allNfts = await getOwnedNfts({
      owner: id,
      filter,
      skip: limit * page,
      limit,
  ...
  useEffect(() => {
    (async () => {
      ...
      const ownedNftCountInfo = await getOwnedNftCounts(id);
      setCountInfo(ownedNftCountInfo);
    })();
  }, [client]);
