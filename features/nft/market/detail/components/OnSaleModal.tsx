...
const OnSaleModal = ({ collectionId, id}) => {
  ...
  const startSale = async(e) => {
    ...
    let duration_type:DurationType = {"startTime": Math.round(new Date(startDate).getTime()/1000), "endTime": Math.round(new Date(endDate).getTime()/1000)}
    ...
    if (sellType==SALE_TYPE[0]){
      ...
      msg = {
        "start_sale": 
        {
          ...
          "initial_price": parseInt(toMinDenom(parseFloat(price), paymentTokens[sellingPriceSelectedOption].denom)).toString(), 
          "reserve_price": parseInt(toMinDenom(parseFloat(reserverPrice), paymentTokens[sellingPriceSelectedOption].denom)).toString(),
          denom
        }
      }
      console.log("msg", JSON.stringify(msg))
  ...
  const PaymentTokenSelectMemo = useMemo(() => {
    ...
    return (
      paymentTokens !== undefined && paymentTokens.length > 0 && <Select
            ...
            <Stack spacing={isMobile() ? 3 : 10}>
              ...
              <Stack direction="row" alignItems="center" marginTop="20px">
                <Stack spacing={8} style={{ padding: '5px 0' }} width="100%">
                  ...
                  >
                    <Stack width={!(sellType == SALE_TYPE[1]) || isMobile() ? '100%' : '50%'}>
