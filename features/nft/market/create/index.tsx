...
export const CreateNFT = () => {
  ...
  const createNFT = async (e) => {
    ...
    if (data.nft == "") {
      toast.warning(`Please upload a nft file.`, {
        position: "top-right",
        autoClose: 5000,
        hideProgressBar: true,
        closeOnClick: true,
    ...
    let dd = createdDate.getUTCDate().toString().padStart(2, "0");
    let mm = createdDate.getUTCMonth().toString().padStart(2, "0");
    let yy = createdDate.getUTCFullYear();
    jsonData["createdDate"] = dd + "-" + mm + "-" + yy;
    const pinataJson = {
      pinataMetadata: {
        name: name,
      },
  ...
  return (
    <Container>
      ...
      <ItemContainer className="collection-item">
        ...
        {/* , MP4, WEBM, MP3, WAV, OGG, GLB, GIFT */}
        <p>File Types Supported: JPG, PNG, GIF, SVG. Max size: 100MB</p>
          ...
          <Select value={collectionId} onChange={handleCollectionIdChange}>
            ...
            {nftcollections.length > 0 &&
              nftcollections.map((collection, idx) => (
                <option value={collection.id} key={`collection${idx}`}>
                  {collection.name}
                </option>
              ))}
          </Select>
