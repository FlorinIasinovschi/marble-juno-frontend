...
const Explore = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      // count of each type
      setDirectSellNFTs(sales.filter(sale => sale.sale_type === "Fixed").length)
      setAuctionNFTs(sales.filter(sale => sale.sale_type === 'Auction').length)
  ...
  useEffect(() => {
    (async () => {
      ...
      if(filter === 'All') {
        ...
        currentTraits = loadedNfts.filter(nft => Object.keys(nft.sale).length === 0)
      } else {
        currentTraits = loadedNfts.filter(nft => nft.sale.sale_type === filter)
      }
